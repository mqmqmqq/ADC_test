...
with openDwfDevice(dwf) as device:
    ...
    #扫描电压
    ntot = 8    #单个变量扫描的点数，总共是(ntot+1)^2
    x = np.zeros(ntot + 1)
    y = np.zeros(ntot + 1)
    z = np.zeros((ntot+1,ntot+1))
    for i in range(ntot+1):
        v_l = 0.55 + (0.7-0.55)/ntot*i
        x[i] = v_l
        spsmu.write("sour:volt 8, %f" % v_l)    #第一个电压或电流
        for j in range(ntot+1):
            v_s = 0.55 + (0.7-0.55)/ntot*j
            spsmu.write("sour:volt 9, %f" % v_s)    #第二个电压或电流
            print(v_l)
            print(v_s)
